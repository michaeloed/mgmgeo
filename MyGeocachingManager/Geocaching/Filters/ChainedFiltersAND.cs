...
namespace MyGeocachingManager.Geocaching.Filters
{
    ...
    /// </summary>
    public class ChainedFiltersAND : CacheFilter
    {
        ...
        public ChainedFiltersAND(CheckedListBox.CheckedItemCollection items)
        {
        ...
        public ChainedFiltersAND(CheckedListBox.CheckedItemCollection items, CacheFilter fltForPlaceholder)
        {
            if ((items != null) && (items.Count != 0))
            {
                filters = new CacheFilter[items.Count];
        ...
        public ChainedFiltersAND(List<CacheFilter> items)
        {
            if ((items != null) && (items.Count != 0))
            {
                filters = new CacheFilter[items.Count];
        ...
        /// </summary>
        /// <param name="items">List of CacheFilter to be used (order will defines order of execution)</param>
        public ChainedFiltersAND(CacheFilter[] items)
        {
        	if ((items != null) && (items.Count() != 0))
            {
        		filters = new CacheFilter[items.Count()];
        ...
        public override bool ToBeDisplayed(Geocache cache)
        {
            ...
            {
                if ((filter != null)&&(filter.ToBeDisplayed(cache) == false))
                    return false;
            }
            return true;
        }
