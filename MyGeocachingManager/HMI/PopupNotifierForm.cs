...
namespace MyGeocachingManager
{
    ...
    internal class PopupNotifierForm : System.Windows.Forms.Form
    {
        ...
        private void PopupNotifierForm_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (Parent.ShowGrip && (Parent.ImgGrip != null))
            {
                e.Graphics.DrawImage(Parent.ImgGrip, (int)((this.Width - Parent.ImgGrip.Width) / 2), (int)((Parent.HeaderHeight - 3) / 2));
            }
            if (Parent.ShowCloseButton)
            {
                if (mouseOnClose)
            ...
            this.Cursor = (Parent.IsClickable() && mouseOnLink) ? Cursors.Hand : Cursors.Default;
            Brush brushText = (Parent.IsClickable() && mouseOnLink) ? brushLinkHover : brushContent;
